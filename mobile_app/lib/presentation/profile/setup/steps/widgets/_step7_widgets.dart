...
class JobEducationHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        Text(
          localizations.translate('step7_title'),
        ...
        Text(
          localizations.translate('step7_description'),
...
class JobIndustrySelector extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final selectedLabel =
        step7ViewModel.jobIndustryOptions.firstWhere(
          ...
          Container(
            ...
            child: DropdownButtonHideUnderline(
              child: DropdownButton<String>(
                ...
                itemHeight: 48,
                value:
                    step7ViewModel.jobIndustryId?.isNotEmpty == true
                        ? step7ViewModel.jobIndustryId
                        : null,
                isExpanded: true,
                hint: const Text(
                  'Select job industry',
                ...
                items:
                    step7ViewModel.jobIndustryOptions.map((option) {
                      final isSelected =
                          step7ViewModel.jobIndustryId == option['value'];
                      return DropdownMenuItem<String>(
                        value: option['value'],
                        child: Row(
                          children: [
                            Text(
                              _getJobEmoji(option['label']!),
...
class EducationLevelSelector extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final selectedLabel =
        step7ViewModel.educationLevelOptions.firstWhere(
          (option) => option['value'] == step7ViewModel.educationLevelId,
          orElse: () => {'value': '', 'label': ''},
        )['label'] ??
        '';
    final emoji =
        selectedLabel.isNotEmpty ? _getEducationEmoji(selectedLabel) : '🎓';
    return Padding(
      padding: const EdgeInsets.only(bottom: 12),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
          ...
          Container(
            ...
            child: DropdownButtonHideUnderline(
              child: DropdownButton<String>(
                ...
                itemHeight: 48,
                value:
                    step7ViewModel.educationLevelId?.isNotEmpty == true
                        ? step7ViewModel.educationLevelId
                        : null,
                isExpanded: true,
                hint: const Text(
                ...
                items:
                    step7ViewModel.educationLevelOptions.map((option) {
                      final isSelected =
                          step7ViewModel.educationLevelId == option['value'];
                      return DropdownMenuItem<String>(
                        value: option['value'],
                        child: Row(
                          children: [
                            Text(
                              _getEducationEmoji(option['label']!),
                              style: const TextStyle(fontSize: 22),
                            ),
                            const SizedBox(width: 12),
                            Text(
                ...
                onChanged: (value) {
                  if (value != null) {
                    final option = step7ViewModel.educationLevelOptions
                        .firstWhere(
                          (opt) => opt['value'] == value,
                          orElse: () => {'value': '', 'label': ''},
                        );
