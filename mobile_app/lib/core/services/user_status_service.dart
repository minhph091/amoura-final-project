...
class UserStatusService {
  ...
  void _handleStatusUpdate(Map<String, dynamic> statusData) {
    try {
      ...
      if (userId != null && status != null) {
        ...
        debugPrint(
          'UserStatusService: User $userId is now ${isOnline ? "online" : "offline"}',
        );
      }
    } catch (e) {
  ...
  Future<bool> getUserOnlineStatus(String userId) async {
    try {
      ...
      debugPrint(
        'UserStatusService: Fetched status for user $userId: ${isOnline ? "online" : "offline"}',
      );
      return isOnline;
    } catch (e) {
      debugPrint(
        'UserStatusService: Error getting user status for $userId: $e',
      );
  ...
  Future<void> updateMyStatus(bool isOnline) async {
    try {
      ...
      debugPrint(
        'UserStatusService: Updating my status to ${isOnline ? "online" : "offline"}',
      );
