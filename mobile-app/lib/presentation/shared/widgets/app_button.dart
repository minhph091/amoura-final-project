...
class AppButton extends StatelessWidget {
  ...
  final Widget? loadingWidget; // Thay loading thành loadingWidget để tránh nhầm lẫn
  ...
  const AppButton({
    ...
    this.borderSide,
    this.loadingWidget, // Widget tùy chỉnh cho trạng thái loading
  });
  ...
  Widget build(BuildContext context) {
    ...
    final Gradient? effectiveGradient = (isDisabled || isLoading)
        ? LinearGradient(
            colors: [
    ...
    final Color effectiveTextColor = (isDisabled || isLoading)
    ...
    final OutlinedBorder effectiveShape = shape ??
        RoundedRectangleBorder(
          borderRadius: BorderRadius.circular(height != null ? (height! / 2) : 26),
        );
    ...
    // Add icon if present and not loading
    if (icon != null && !isLoading) {
      buttonContent = Row(
        ...
        children: [
          Icon(
            icon,
            size: (buttonTextStyle.fontSize ?? 16) * 1.25,
            color: effectiveTextColor,
          ),
    ...
    if (effectiveGradient != null && !isOutline && borderSide == null) {
      return Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [
            if (effectiveElevation > 0 && !isDisabled && !isLoading)
              BoxShadow(
